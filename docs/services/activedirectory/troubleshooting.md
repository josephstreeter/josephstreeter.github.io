...
function Test-ADHealth {
    ...
    try {
        ...
        $HealthReport.Tests.DCConnectivity = @{
            Status = "Pass"
            Details = "Successfully connected to DC"
        }
    }
    catch {
        $HealthReport.Tests.DCConnectivity = @{
    ...
    # Test replication
    try {
        $replStatus = repadmin /showrepl $DomainController
        if ($replStatus -match "error|fail") {
            $HealthReport.Tests.Replication = @{
                Status = "Fail"
                Details = "Replication errors detected"
            }
        }
        else {
            $HealthReport.Tests.Replication = @{
                Status = "Pass"
                Details = "Replication functioning normally"
            }
        }
    }
    catch {
    ...
    # Test DNS
    try {
        $dnsTest = Resolve-DnsName -Name "_ldap._tcp.$Domain" -Type SRV -ErrorAction Stop
        $HealthReport.Tests.DNS = @{
            Status = "Pass"
            Details = "DNS SRV records resolved successfully"
        }
    }
    catch {
        $HealthReport.Tests.DNS = @{
    ...
    try {
        ...
        if (Test-Path $sysvolPath) {
            $HealthReport.Tests.SYSVOL = @{
                Status = "Pass"
                Details = "SYSVOL share accessible"
            }
        }
        else {
            $HealthReport.Tests.SYSVOL = @{
        ...
        $HealthReport.Tests.SYSVOL = @{
            Status = "Error"
            Details = "Error testing SYSVOL: $($_.Exception.Message)"
        }
    }
...
function Test-ServicePrincipalNames {
    ...
    try {
        $spns = Get-ADUser -Identity $ServiceAccount -Properties ServicePrincipalNames
        
        $spnInfo = @{
            Account = $ServiceAccount
            SPNs = $spns.ServicePrincipalNames
            Issues = @()
        }
        
        # Check for duplicate SPNs
        foreach ($spn in $spns.ServicePrincipalNames) {
            $duplicates = Search-ADAccount -SearchBase (Get-ADDomain).DistinguishedName -LDAPFilter "(servicePrincipalName=$spn)"
            if ($duplicates.Count -gt 1) {
                $spnInfo.Issues += "Duplicate SPN found: $spn"
            }
        }
...
function Test-ADReplication {
    ...
    foreach ($dc in $DomainControllers) {
        Write-Output "Checking replication for $dc..."
        ...
        try {
            ...
            $inboundRepl = repadmin /showrepl $dc /csv | ConvertFrom-Csv
            foreach ($repl in $inboundRepl) {
                if ($repl.'Last Success Time' -eq "0" -or $repl.'Consecutive Failures' -gt 0) {
                    $dcReport.Issues += "Inbound replication issue from $($repl.'Source DSA')"
                }
            }
            
            # Check outbound replication
            $outboundRepl = repadmin /showrepl $dc /csv /all | ConvertFrom-Csv
            foreach ($repl in $outboundRepl) {
                if ($repl.'Last Success Time' -eq "0" -or $repl.'Consecutive Failures' -gt 0) {
                    $dcReport.Issues += "Outbound replication issue to $($repl.'Destination DSA')"
                }
            }
            
            $replicationReport.DomainControllers[$dc] = $dcReport
        }
        catch {
            $dcReport.Issues += "Failed to check replication: $($_.Exception.Message)"
            $replicationReport.DomainControllers[$dc] = $dcReport
        }
    }
