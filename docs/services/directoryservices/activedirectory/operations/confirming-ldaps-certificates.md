...
| `EE certificate key too weak` (num=66) | The DC certificate's key is below the accepted strength (e.g. RSA < 2048-bit) | Reissue the DC certificate with an RSA key ≥ 2048-bit (4096-bit recommended) or an ECC key ≥ 256-bit. This is a key-strength failure, **not** a trust/self-signed issue. |
