...
- **[RFC 4880 - OpenPGP Message Format](https://datatracker.ietf.org/doc/html/rfc4880)** - The official OpenPGP specification
- **[RFC 6637 - Elliptic Curve Cryptography (ECC) in OpenPGP](https://datatracker.ietf.org/doc/html/rfc6637)** - ECC support in OpenPGP
- **[RFC 3156 - MIME Security with OpenPGP](https://datatracker.ietf.org/doc/html/rfc3156)** - Email integration standards
...
Run this checklist before publishing changes to the PGP section:

1. **Link quality**

- Validate new and existing external links are reachable.
- Prefer canonical vendor/spec URLs over mirrors.
- Replace or remove stale links.

1. **Security accuracy**

- Remove unsafe command patterns (hardcoded passphrases, weak digests, insecure key handling).
- Ensure examples use modern defaults and clearly scoped exceptions.
- Verify no plaintext-sensitive workflow is presented as acceptable.

1. **Threat-model consistency**

- Keep enterprise integrity-first and anonymity-focused guidance clearly separated.
- Ensure signing recommendations are consistent with stated threat model.
- Avoid contradictory defaults across chapters.

1. **Terminology and ecosystem currency**

- Mark legacy tools as legacy.
- Update deprecations and replaced features.
- Verify keyserver and standards references are current.

1. **Structure and style**

- Remove placeholder text and duplicate sections.
- Keep frontmatter schema consistent with section conventions.
- Confirm heading hierarchy and formatting pass markdown lint rules.

1. **Cross-chapter integrity**

- Validate all internal links and next-step links.
- Ensure chapter transitions do not repeat or conflict.
- Confirm diagrams and examples align with narrative guidance.

1. **Release metadata**

- Update `last_updated` in changed files.
- Update this file's review date footer.
