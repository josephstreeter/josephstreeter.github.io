...
A comprehensive backup strategy ensures data protection and rapid recovery from failures, data corruption, or disasters. This page covers Prometheus and Alertmanager backups along with the shared backup infrastructure used across the whole monitoring stack — backup storage (S3, MinIO, encryption), disaster recovery testing, backup monitoring, and the combined automation scripts. Grafana-specific backup and restore procedures are documented in [Grafana Backup and Recovery](../grafana/backup-recovery.md).
...
# Create the snapshot with a single POST and capture its response
# (requires the --web.enable-admin-api flag). Calling the endpoint twice
# would create two separate snapshots and throw the first one away.
...
The storage, encryption, and off-site upload procedures below are shared infrastructure used by every backup in the monitoring stack, including Grafana. The [Grafana Backup and Recovery](../grafana/backup-recovery.md) page references this section rather than duplicating it.
...
The combined automation below orchestrates the Prometheus, Grafana, and Alertmanager backup scripts for the entire monitoring stack.
...
- [Grafana Backup and Recovery](../grafana/backup-recovery.md)
- [Monitoring Stack Overview](../index.md)
- [Prometheus Overview](index.md)
