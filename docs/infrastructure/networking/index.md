---
title: "Networking"
description: "Enterprise networking documentation covering fundamentals, design, automation, and operations"
author: "Joseph Streeter"
tags: ["networking", "infrastructure", "network-design", "network-automation"]
category: "infrastructure"
difficulty: "beginner-to-advanced"
last_updated: "2025-12-29"
---

Welcome to the comprehensive networking documentation hub. Whether you're just starting with networking concepts or managing enterprise infrastructure, you'll find guides, tutorials, and reference materials organized by expertise level.

## Quick Navigation

### Learning Path

**New to networking?** Start here with fundamentals

- [Network Fundamentals](fundamentals.md) - Core concepts and protocols
- [OSI Model Guide](osi-model.md) - Deep dive into the seven layers

### Design & Architecture

**Planning network infrastructure?**

- [Enterprise Architecture](architecture.md) - Design principles and best practices
- [VLAN Strategy](vlans.md) - Segmentation and management

### Configuration & Guides

**Setting up equipment?**

- [Unifi Equipment](unifi/index.md) - UniFi Dream Machine, switches, APs
- [Common Tasks](guides.md) - Step-by-step configuration guides

### Automation & Operations

**Automating network tasks?**

- [PowerShell Automation](automation.md) - Network management scripts
- [Monitoring & Troubleshooting](troubleshooting.md) - Operations guide

## Core Concepts

Master the fundamental building blocks of networking:

- [Network Fundamentals](fundamentals.md) - IP addressing, subnetting, protocols
- [OSI Model](osi-model.md) - Seven-layer reference model
- [Routing Protocols](routing.md) - Static and dynamic routing concepts
- [DNS Service](dns.md) - Domain Name System architecture
- [DHCP Service](dhcp.md) - Dynamic IP address management
- [Wireless Networking](wireless.md) - 802.11 standards and best practices

## Infrastructure Design

Plan and architect enterprise-grade networks:

- [Enterprise Architecture](architecture.md) - Three-tier hierarchical design
- [VLAN Strategy](vlans.md) - Network segmentation and management
- [Firewall Design](firewalls.md) - Security architecture and implementation

## Configuration & Operations

Implement and manage network infrastructure:

- [Configuration Guides](guides.md) - Platform-agnostic setup procedures
- [Troubleshooting](troubleshooting.md) - Diagnostic methodologies
- [Automation](automation.md) - PowerShell network management scripts

## Platform-Specific Implementation

Vendor-specific configuration guides:

- [Cisco Networking](cisco/index.md) - IOS configuration, VLANs, routing
- [UniFi Network](unifi/index.md) - Dream Machine, switches, access points

## Security

Protect and secure network infrastructure:

- [Network Security](security/index.md) - Security frameworks and automation

## Reference Materials

- [Networking Glossary](glossary.md) - Comprehensive terminology reference
- [Getting Started](getting-started.md) - Your first network setup

## Reference

### Commands Cheat Sheet

### RFC Standards

### Best Practices

## Related Topics

- [Network Security](security/index.md)
- [Infrastructure Monitoring](../monitoring/index.md)
- [Windows Infrastructure](../windows/index.md)
- [Container Networking](../containers/index.md)
- [Disaster Recovery](../disaster-recovery/index.md)
